...
- **NEVER use random or stratified k-fold CV when the dataset has a timestamp column or any temporal ordering.** Random/stratified CV silently overstates performance on time-ordered data because (a) entity-level leakage (same device/IP appears in train and test folds), and (b) concept drift in target distributions (P(y|x) changes between train and test windows) is invisible to the metric. The first experiment on any dataset that *could* be time-ordered MUST use a chronological holdout (HoldoutSplit with `order: time`) or `time_series_split`/`walk_forward`. If the user has explicitly verified their data is i.i.d. and not time-dependent, document the verification in the project CLAUDE.md before allowing any random/stratified CV. Reference: Bergmeir, Hyndman & Koo 2018 IJF (arXiv:1905.11744). This rule was added 2026-04-24 after the fraud_ecommerce benchmark exposed a 0.27 AUC gap between stratified-CV (0.7738, inflated) and chronological-holdout (0.5098, honest) results on the SAME XGBoost config and SAME 151,112-row dataset.
- **Velocity / target-encoding features for entity-level aggregations MUST be computed from train-period rows only.** Compute the train-period boundary as `n_train = n_total - n_val - n_test` to match the runner's HoldoutSplit slicing exactly; if you compute from `n * train_fraction` you will leak validation rows into their own predictor values, producing val_AUC ≫ test_AUC (the diagnostic signal). Reference: Kaufman, Rosset, Perlich & Stitelman 2012 ACM-TKDD on leakage formulation.
