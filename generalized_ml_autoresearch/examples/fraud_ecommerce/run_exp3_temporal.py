"""Run Exp 3 — chronological holdout + cyclical temporal features."""
...
def main():
    ...
    cfg["description"] = "XGBoost + cyclical temporal features (hour, dow, signup_hour) on chronological holdout"

    results_dir = Path(cfg["paths"]["results_dir"])
    results_dir.mkdir(parents=True, exist_ok=True)
    (results_dir / "trade_logs").mkdir(exist_ok=True)

    seed = json.loads((HERE / "seed_reasoning_exp3.json").read_text(encoding="utf-8"))
    ...
    print("\n--- Summary (Exp 3 — chronological + temporal features) ---")
    ...
    print(f"  vs Exp 2 (chronological, no temporal): test AUC = 0.5098")
