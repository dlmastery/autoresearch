"""Run Exp 6 — chronological holdout + velocity features (leakage fixed)."""
...
def main():
    cfg = _load_config(HERE / "config_exp6.yaml")
    results_dir = Path(cfg["paths"]["results_dir"])
    results_dir.mkdir(parents=True, exist_ok=True)
    (results_dir / "trade_logs").mkdir(exist_ok=True)

    seed = json.loads((HERE / "seed_reasoning_exp6.json").read_text(encoding="utf-8"))
    ...
    print("\n--- Summary (Exp 6 — velocity features, leakage fixed) ---")
    ...
    print(f"  vs Exp 5 (leaked):  test=0.5297 val=0.9988")
    print(f"  vs Exp 2 baseline:  test=0.5098")
    print(f"  vs FDB AFD TFI:     test=0.636")
