...
def _make_heads(hidden_size: int, dropout: float = 0.1, het_loss: bool = True) -> nn.ModuleDict:
...
class CurrencyMLP(nn.Module):
    def __init__(self, n_input_features: int, seq_len: int = 10, hidden_size: int = 512, head_dropout: float = 0.1, het_loss: bool = True):
        super().__init__()
...
def create_model(
    ...
    if backbone == "mlp":
        return CurrencyMLP(n_input_features, seq_len=seq_len, head_dropout=head_dropout, het_loss=het_loss)
    elif backbone == "lstm":
        return CurrencyLSTM(n_input_features, head_dropout=head_dropout, het_loss=het_loss)
