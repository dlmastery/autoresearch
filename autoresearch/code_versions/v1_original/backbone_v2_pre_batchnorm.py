...
class CurrencyMLP(nn.Module):
    def __init__(self, n_input_features: int, seq_len: int = 10, hidden_size: int = 128, head_dropout: float = 0.1, het_loss: bool = True):
        super().__init__()
        self.het_loss = het_loss
        head_hidden = min(hidden_size, 64)  # scale head with backbone
